...
class CalculateWeeklyLeagueMatchups
{
    ...
    public function __construct(WeeklyLeagueMatchUps $matchups)
    {
    ...
    public function matchups_exists_for_fixture($new_fixture_id): bool
    {
        ...
        if (isset($all_matchups['fixture_id_' . $new_fixture_id])) {
            return true;
        }
    ...
    public function previous_matchups_exists( $new_fixture_id ){
        ...
        if(!empty($this->previous_matchups)){
            return true;
        }
    ...
    public function for_fixture_id( int $new_fixture_id)
    {
        ...
        $previous_matchups = $this->get_previous_matchups($new_fixture_id);
        ...
        $next_matchups = array(
            'fixture_id_' . $new_fixture_id => $next,
        );
    ...
    protected function initialize_matchups($fixture_id)
    {
        ...
        $leagueCupCompetition = new LeaguesCupCompetition(get_post($this->competition_id));
        $participants_ids = $leagueCupCompetition->participants;

        if( count($participants_ids) < 4 ){
            $next_matchups = 'not enough players';
            error_log(__METHOD__ . ' not enough players for LeaguesCupCompetition: ' . $this->competition_id);
            $this->$next_matchups = $next_matchups;
            return;
        }
        ...
        if( count($participants_ids) % 2  !== 0 ){
            $next_matchups = 'not enough players';
            error_log(__METHOD__ . ' not even number of players for LeaguesCupCompetition: ' . $this->competition_id);
            $this->$next_matchups = $next_matchups;
            return;
        }
        ...
        $next_matchups = array(
            'fixture_id_' . $fixture_id => $participants_ids
        );

        error_log(__METHOD__ . ' initialize matchups fixture: ' . $fixture_id . ' pairs: ' . json_encode($next_matchups));
